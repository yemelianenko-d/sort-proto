...
Рівні механіки: `MechanicModule.levels?` — generic `JsonLevelSource`
(`core/game/JsonLevelSource.ts`: fetch + парсер механіки + standalone-глобал).
Сортинг grandfathered на core `LevelManager`; нові механіки дають свій source
(див. `mechanics/blocks/blocksLevels.ts`). Preloader вантажить рівні всіх
модулів; лоббі mechanic-aware: до master-lobby активна механіка обирається
URL-флагом **`?mechanic=<id>`** (дефолт — сортинг), `?level=N` працює в
контексті активної механіки.
