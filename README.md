...
Спецблоки, що лишились, обмежують простір і рух, але не ламають арифметику
сетів: чорнильна пляма (нерухомий мертвий слот унизу колонки — займає місце,
не збирається і не входить у win condition), ключ у завалі, скотч. Замкнена (з ключами в завалі) і ланцюгова колонки не порожні: всередині
видно 2-3 замкнені кольорові блоки — без них рівень не зібрати, тож відкриття
колонки необхідне за побудовою на кожному рівні (booster-only замки лишаються
порожнім бонусом, щоб не примушувати платити бустером). Нові механіки поверх
цього: колонка заданого кольору (порожня колонка з
крейдяним візерунком приймає першим тільки свій колір, далі — звичайна),
колонка в ланцюгах (кожен зібраний набір знімає одну ланку в момент
валідації сета: сірий ланцюг знімає будь-який набір, кольоровий — тільки
набір свого кольору; кольоровий ланцюг завжди кольору, що існує на рівні), подвійний замок (одна колонка, N ключів; кожен ключ —
бустерний чи з завалу — знімає один замок; щойно всі замки зняті, ключі-блоки,
що лишились на дошці, розчиняються як мертвий вантаж — прихований спершу
перевертається лицем, потім тане; після undo бустерні відкриття не
...
бустер даремно. Крива (150 рівнів, декларативна таблиця CURVE у SortingLevelGenerator.ts):
1-10 навчальна (кураторська, приховані блоки з 4, бустер-замок з 7),
11-15 чиста база, чорнило з 16, ключ-блок з 21, чорнило+ключ 26-30, скотч з 31,
колонка кольору з 36, ланцюги з 41 (кольоровий з 44), подвійний замок з 46.
З 51 — ротація у трьох тірах тиску (51-80 один якір + легка приправа,
81-110 пари механік, 111-150 щільні комбо з x2-варіантами); якір механіки
розподіляється збалансованою ротацією по декадах (кожна механіка гарантовано
в кожній декаді), перед кожною новою декадою — рівень-видих. Солвер тепер в
окремому модулі (SortingSolver.ts) і уточнює par обмеженими до-пошуками.
Звіт розподілу механік по декадах друкує `npx tsx tools/build-levels.ts`. У чит-режимі (перемикач у Налаштуваннях) клітинки лоббі показують гліфи
механік рівня: I чорнило, K ключі (K2 — два), L замок без ключа, T скотч,
C колонка кольору, S ланцюги.

## Дудли (декор полів)

Два пули: `deco_doodle_01..30` — універсальні, доступні будь-якій сцені будь-якої
механіки; `deco_sort_01..10` — рукописні фрази про сортування, передаються в
`scatterDoodles(..., extraKeys)` **тільки зі сцен механіки сортування** і не
мають з'являтись в інших механіках. Нові механіки заводять власний пул
`deco_<mechanic>_*` за тим самим патерном.
