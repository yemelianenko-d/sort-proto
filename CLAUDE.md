...
- **Puzzle Hub — багатомеханічний скелет.** Проект стає колекцією механік на
  ...
  **Наступний інкремент — split ассетів на `shared` + `mechanics/sorting` з
  префіксами ключів** (ще не зроблено).
...
**Наступні кроки:**
- **Puzzle Hub інкремент 2 — split ассетів**: розкласти `public/assets/images/`
  на `shared/` + `mechanics/sorting/`, розбити `manifest.json` на два, винести
  `SHARED_KEYS`/`SORTING_KEYS`, Preloader вантажить shared, `SortingScene` —
  свій манифест. Тоді `sortingModule.assetManifestUrl` вказати на нього.
